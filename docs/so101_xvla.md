...
| 归一化模式 | **IDENTITY** | MEAN_STD | QUANTILES |
| 默认冻结策略 | **不冻结** | 冻结 VLM | 冻结 VLM |
| 预训练基座 | `lerobot/xvla-base` | `lerobot/smolvla_base` | `lerobot/pi05_base` |
| 依赖 extra | `xvla` | `smolvla` | `pi` |

### 策略选择：Phase II 领域自适应 (PEFT)

为了在显存有限（如 8GB 显存）的设备上进行微调，并利用 X-VLA 的泛化能力，我们采用 **Phase II 领域自适应** 策略：

* **冻结**：Vision Encoder、Language Encoder 和 Policy Transformer 均被冻结（`freeze_vision_encoder=true`, `freeze_language_encoder=true`, `train_policy_transformer=false`）
* **训练**：仅训练 Soft Prompts 和 Action Head（`train_soft_prompts=true`）。该模式下仅需训练约 9M 参数，大幅降低显存要求。
...
X-VLA 使用 IDENTITY 归一化，不需要运行 `augment_dataset_quantile_stats.py`：

```python
# configuration_xvla.py L60-65
...
这意味着数据集不需要额外的归一化统计量计算，LeRobot 默认数据集格式即可直接使用。
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `policy.path` | `lerobot/xvla-base` | 预训练基座模型 |
| `policy.dtype` | `float32` | 推荐 `bfloat16` 避免 OOM |
| `policy.action_mode` | `ee6d` | **SO-ARM101 必须设为 `auto`** |
| `policy.max_action_dim` | `20` | 模型输出维度，auto 模式 pad 数据集维度到此值 |
| `policy.freeze_vision_encoder` | `false` | 默认不冻结视觉编码器 |
| `policy.freeze_language_encoder` | `false` | 默认不冻结语言编码器 |
| `batch_size` | 32 | 显存不够降至 16/8 |
| `steps` | 20000 | 官方推荐起点 |
...
| `optimizer_betas` | (0.9, 0.99) | Adam beta（注意：与 SmolVLA 的 0.95 不同） |
| `optimizer_weight_decay` | 0.0 | 无 L2 正则 |
...
> X-VLA 即使在 Phase II 领域自适应模式下显存需求也较高，建议针对 8GB 显存设备将 batch size 降至 8 或 16 尝试。
...
X-VLA 推理时使用 **10 步** Flow Matching 去噪（`num_denoising_steps=10`），生成 32 步的动作 chunk（`chunk_size=32`）。
