...
### 5. 解决方案
手动修改并上传数据集后，必须将 `v3.0` tag 移到 main 的最新 commit 上。

**方法一：使用 hf CLI（推荐）**
上传修改后的数据集后，为最新 commit 重新打上 `v3.0` 标签：
```bash
# 如果原来有 v3.0 标签，需要先删除（或者在网页端操作）
hf repos tag delete Atticuxz/so101-table-cleanup v3.0 --yes --type dataset
# 为当前 main 的最新提交打上 v3.0 标签
