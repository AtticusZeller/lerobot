...
7. [数据标注能力（Pi0.5 需要 / X-VLA 不需要）](#7-数据标注能力)
8. [可视化与验证](#8-可视化与验证)
9. [快速参考流程](#9-快速参考流程)
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--dataset.episode_time_s` | 60 | 每条 episode 录制时长（秒）。动作完成后按 **→** 可提前结束，不必等满 |
| `--dataset.reset_time_s` | 60 | episode 间重置等待时长（秒）。按 **→** 可跳过等待 |
| `--dataset.num_episodes` | 50 | 录制总 episode 数 |

> **减少静止帧建议**：根据任务实际时长设置 `episode_time_s`（如 10 秒动作设 15），留少量余量即可，不必用默认 60 秒。
...
> 快捷键在 reset 阶段同样生效：按 **→** 可跳过 reset 等待，直接开始下一条。
