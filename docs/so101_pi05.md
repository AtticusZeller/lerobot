...
Pi0.5 默认使用 **QUANTILES** 归一化（而非 MEAN_STD）。状态值被离散化为 256 个 bin，需要 q01/q99 分位数来确定归一化范围。源码 `configuration_pi05.py:66-72`：
...
大部分 LeRobot 数据集在创建时只包含 mean/std 统计量，**不包含** quantile 统计量。使用 Pi0.5 训练前必须先补充，否则会报错：
